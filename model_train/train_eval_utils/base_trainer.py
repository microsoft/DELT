...
class BaseTrainer():
    ...
    def get_log(self, stats, phase, **kwargs):
        ...
        log_midfix = " | ".join([f"{k}: {v:.6f}" for k,v in stats.items()])
    ...
    def train(self):
        if self.args.do_train:
            train_sampler = DistributedSampler(self.train_dataset, shuffle=((not self.args.precompute_data_order) and (not self.args.no_shuffle)), drop_last=True, rank=self.dp_rank, num_replicas=self.dp_world_size, seed=self.args.order_seed)
