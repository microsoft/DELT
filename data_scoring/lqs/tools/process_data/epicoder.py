...
class Encoder(object): 
    ...
    def encode(self, line):
        instruction = line["instruction"].replace("\n", " ")
        output = line["output"].replace("\n", " ")
        ...
        assert len(output) > 0
        conv = instruction + Encoder.tokenizer.bos_token + output
        conv = instruction + Encoder.tokenizer.eos_token + Encoder.tokenizer.bos_token + output
        # conv = instruction + Encoder.tokenizer.eos_token + Encoder.tokenizer.bos_token + output
        #tokens = Encoder.tokenizer.encode(conv, add_special_tokens=False)
        instruct_tokens = Encoder.tokenizer.encode(instruction, add_special_tokens=False)
        output_tokens = Encoder.tokenizer.encode(output, add_special_tokens=False)
        if Encoder.tokenizer.bos_token_id in instruct_tokens:
            instruct_tokens.remove(Encoder.tokenizer.bos_token_id)
        if Encoder.tokenizer.bos_token_id in output_tokens:
            output_tokens.remove(Encoder.tokenizer.bos_token_id)
        assert Encoder.tokenizer.bos_token_id not in instruct_tokens
        assert Encoder.tokenizer.bos_token_id not in output_tokens
        tokens = instruct_tokens + [Encoder.tokenizer.bos_token_id] + output_tokens
        
        #assert Encoder.tokenizer.bos_token_id in tokens
        # if tokens.count(Encoder.tokenizer.bos_token_id) != 1:
        #     print(tokens.count(Encoder.tokenizer.bos_token_id))
        #     print(instruction)
        #     print(output)
        #     print(tokens)
        # assert tokens.count(Encoder.tokenizer.bos_token_id) == 1
