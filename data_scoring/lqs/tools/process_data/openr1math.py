...
class Encoder(object):
    ...
    def encode(self, id_with_row_data):
        doc_id, row_data = id_with_row_data

        if isinstance(row_data, pd.Series):
            question = row_data["problem"] 
            answer = row_data["solution"] 
        elif isinstance(row_data, dict):  
            question = row_data["problem"] 
            answer = row_data["solution"] 
        else: 
            raise ValueError(f"Unexpected row format: {type(row_data)}")
...
def main():
    ...
    for fid, file_name in enumerate(files_names):
        ...
        if 'problem' not in df.columns or 'solution' not in df.columns:
            raise ValueError(f"Expected 'text' column in {file_name}, but not found.")

        encoded_docs = pool.imap_unordered(encoder.encode, ((doc_id, row) for doc_id, (_, row) in enumerate(df.iterrows())), 50)
