...
def add_runtime_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--order-seed", type=int, default=10)
...
def add_data_annotation_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('data_annotation', 'solve gamma experiments')
    ...
    group.add_argument("--mould-weight", type=float, default=None)

    group.add_argument("--arc-theta1", type=float, default=None)
    group.add_argument("--arc-m", type=float, default=None)
...
def get_args():
    ...
    parser = add_data_annotation_args(parser)
    ...
    if args.type in ["data_annotation"]:
        save_path = os.path.join(
            ...
            + (f"-w{args.mould_weight}" if args.mould_weight is not None else "")
            + (f"-theta{args.arc_theta1}" if args.arc_theta1 is not None else "")
            + (f"-m{args.arc_m}" if args.arc_m is not None else "")
