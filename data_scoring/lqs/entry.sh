...
bash $BASE_PATH/data_scorer/lqs/scripts/prepare_full_dataset.sh $DATA_PATH $CONFIG_PATH # tokenize cc use fairseq 125M

# Step 2: prepare target dataset.
bash $BASE_PATH/data_scorer/lqs/scripts/prepare_target_dataset.sh $CONFIG_PATH # tokenize lima

# Step 3: prepare (sampling and annotation) training dataset for scoring model.
# Step 3.1: proxy data sampling.
bash $BASE_PATH/data_scorer/lqs/scripts/proxy_data_sampling.sh $CONFIG_PATH # sample proxy data from cc
# Step 3.2: proxy data annotation.
bash $BASE_PATH/data_scorer/lqs/scripts/proxy_data_annotation.sh $CONFIG_PATH # proxy data annotation
