...
class GammaTrainer(BaseTrainer):
        ...
        for i in range(end-1, start-1, -1):
            ...
            if self.args.compute_ct_interval is None or i % self.args.compute_ct_interval == 0:
                if self.args.mould_weight != None or self.args.arc_theta1!= None:
                    ...
                    ct, lam_norms, grad_norms, cos_thetas, thetas = self.compute_proxy_ct_vis(lam_param, params, buffers, self.args.mould_weight, self.args.arc_theta1, self.args.arc_m)
                else:
    ...
    def compute_proxy_ct(self, lam_param, params, buffers, w=None, theta1=None, m=None):
        ...
        for i, (model_batch, no_model_batch) in enumerate(proxy_dataloader):
            ...
            ct, updated_grad_checkpoint = jvp_batch(
                self.model, batch, lam_param, params, buffers,
                w=w, theta1=theta1, m=m, grad_checkpoint=batch_grad_checkpoint)
    ...
    def compute_proxy_ct_vis(self, lam_param, params, buffers, w=None, theta1=None, m=None):
        '''
        Compute the contribution of the proxy data to the gradient of the loss w.r.t. the model parameters.
        '''
        proxy_sampler = DistributedSampler(
        ...
        for i, (model_batch, no_model_batch) in enumerate(proxy_dataloader):
            ...
            ct, lam_norm, grad_norm, cos_theta, theta = jvp_batch(
                self.model, batch, lam_param, params, buffers,
                w=w, theta1=theta1, m=m, grad_checkpoint=batch_grad_checkpoint)

            ct = all_gather(ct, dim=1, op="stack").view(-1)
            lam_norm = all_gather(lam_norm, dim=1, op="stack").view(-1)
            grad_norm = all_gather(grad_norm, dim=1, op="stack").view(-1)
            cos_theta = all_gather(cos_theta, dim=1, op="stack").view(-1)
            theta = all_gather(theta, dim=1, op="stack").view(-1)
            
            ct = self.args.lr * ct

            all_ct.append(ct)
            lam_norms.append(lam_norm)
        ...
        all_ct = torch.cat(all_ct, dim=0)
        lam_norms = torch.cat(lam_norms, dim=0)
        grad_norms = torch.cat(grad_norms, dim=0)
        cos_thetas = torch.cat(cos_thetas, dim=0)
        thetas = torch.cat(thetas, dim=0)

        all_ct = all_ct[:len(self.proxy_dataset)]
        lam_norms = lam_norms[:len(self.proxy_dataset)]
        grad_norms = grad_norms[:len(self.proxy_dataset)]
        cos_thetas = cos_thetas[:len(self.proxy_dataset)]
        thetas = thetas[:len(self.proxy_dataset)]

        return all_ct, lam_norms, grad_norms, cos_thetas, thetas
